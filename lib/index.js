...
export function apply(ctx, config) {
    ...
    const adapter = new TokenRhythmAdapter({
        ...
        routingModelId: ROUTING_MODEL_ID,
        isPubliclySelectable: () => options().routingEnabled,
        resolveApiKey,
        resolveUserId,
    ...
    let routingTraceFn = () => [];
    const installedRouting = installRouting(ctx, () => options().routing, ROUTING_MODEL_ID, historyStore, modelFacts, () => options().connection.defaultContextWindow, { info: message => ctx.logger.info(message), warn: message => ctx.logger.warn(message) });
    routingTraceFn = sessionId => sessionId === undefined ? [] : installedRouting.trace(sessionId);
